...
class OrbitControls extends EventDispatcher {
  constructor(object, domElement) {
    ...
    function handleTouchStartRotate(event) {
      scope.dampingFactor = 1
      if (pointers.length === 1) {
        rotateStart.set(event.pageY, event.pageX)
      } else {
    ...
    function handleTouchMoveRotate(event) {
      ...
      rotateUp(-(2 * Math.PI * rotateDelta.x) / element.clientHeight) // yes, height

      rotateLeft((2 * Math.PI * rotateDelta.y) / element.clientHeight)
